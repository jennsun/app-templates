...
1. **Do they already have deployed apps to test, or do they need to set up mock apps first?**
2. **What compute sizes do they want to test?** (Medium, Large, or both)
3. **How many worker configurations do they want to test?** (e.g., 2, 4, 6, 8 workers)
4. **Do they have M2M OAuth credentials (service principal client_id/client_secret)?** — Required for long-running tests. If not, guide them to create one.
5. **What is their `DATABRICKS_HOST`?** (workspace URL)
...
## Step 2: Mock Your Agent for Load Testing

The goal is to **mock your LLM and tool calls** so you test the Apps infrastructure throughput — not external API latency.
